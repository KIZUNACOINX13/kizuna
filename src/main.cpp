...
#if defined(NDEBUG)
#error "Kiz cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in ukiz) are considered zero fee (for relaying and mining)
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) {
        ...
        nSubsidy = 6000000000 * COIN; // Premine
    }
    else if (nHeight >= 2 && nHeight <= 6500) {
        nSubsidy = 1388 * COIN; // Pow phase
    }
    else if (nHeight >= 6501 && nHeight <= 7925072) {
        nSubsidy = 60 * COIN; // Pos phase
    } else {
        nSubsidy = 30 * COIN; // Halving at 11 000 000 000 supply
    }
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
    ...
    ret = blockValue * 0; // No masternode rewards
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // KIZ: We use certain sporks during IBD, so check to see if they are
